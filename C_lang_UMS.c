...
void initializeSystem()
{
    ...
    for (int i = 0; i < MAX_STUDENTS; i++)
    {
        ...
        float calculateCourseGPA(int student_idx, int course_idx)
        {
            if (marks[student_idx][course_idx] == -1)
                return 0;
            int maxMarks = atoi(courses[course_idx][2]);
        ...
        void displayCourseReport()
        {
            ...
            for (int i = 0; i < student_count; i++)
            {
                if (strcmp(enrolled[i][course_idx], "") != 0 && marks[i][course_idx] != -1)
                {
                    ...
                    if (studentMarks > highest)
                        highest = studentMarks;
                    if (studentMarks < lowest)
                        lowest = studentMarks;
            ...
            for (int i = 0; i < course_count; i++)
            {
                ...
                if (enrolledCount > 0)
                {
                    ...
                    if (avgMarks >= 80)
                        ...
                        strcpy(performance, "AVERAGE");
                    else if (avgMarks >= 50)
                        strcpy(performance, "POOR");
                    else
                        strcpy(performance, "VERY POOR");
